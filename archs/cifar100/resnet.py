...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_block, num_classes=100, k=64):
        ...
        # we use a different inputsize than the original paper
        # so conv2_x's stride is 1
        conv2_x = self._make_layer(block, k, num_block[0], 1)
        conv3_x = self._make_layer(block, 2*k, num_block[1], 2)
        conv4_x = self._make_layer(block, 4*k, num_block[2], 2)
        conv5_x = self._make_layer(block, 8*k, num_block[3], 2)
        avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        fc = nn.Linear(8 * k * block.expansion, num_classes)
        self.features = nn.Sequential(
            conv1, conv2_x, conv3_x, conv4_x, conv5_x, avg_pool)
        # self.features = nn.Sequential(
