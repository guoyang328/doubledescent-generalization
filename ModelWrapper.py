...
class ModelWrapper(object):
    ...
    def calculate_measurements(self, train_loader, val_loader, criterion, dataset_name):
        margin = self.calculate_margin(val_loader, criterion)
        measures, bounds = measure.calculate(
            self.model, self.initial_model, self.device, train_loader, margin, dataset_name)
        return measures, bounds
    # =====================================================

    # =====================================================
    # calculate margins
    def calculate_margin(self, val_loader, criterion):
        ...
        def validate(model, device, val_loader, criterion):
            ...
            with torch.no_grad():
                for i, (data, target) in enumerate(val_loader):
                    # data, target = data.to(device).view(
                    #     data.size(0), -1), target.to(device)
                    ...
                    margin = torch.cat(
                        (margin, output[:, target].diag() - output_m[:, output_m.max(1)[1]].diag()), 0)
        ...
        _, _, margin = validate(self.model, self.device, val_loader, criterion)
        return margin
    # =====================================================
    ...
    def get_optimization_var(self, data_loader, max_num=1000):
        ...
        ov = np.array([np.mean(np.var(logits[:, _, :], axis=0))
